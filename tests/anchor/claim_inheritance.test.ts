...
describe("claim_inheritance", () => {
  ...
  // In v2 the beneficiary's Solana pubkey bytes are stored as beneficiaryUtxoPubkey
  // so claim_inheritance can verify the signer matches the stored bytes.
  ...
  beforeEach(async () => {
    ...
    // Store the beneficiary's pubkey bytes as the UTXO pubkey so the on-chain
    // program can verify the claim_inheritance signer matches the stored identity.
  ...
  it("untriggered vault rejected with VaultNotTriggered", async () => {
    ...
    const ben2UtxoPubkey = Array.from(ben2.publicKey.toBytes());
    ...
    await program.methods
      .initializeVault(new BN(0), new BN(5_000_000), ben2UtxoPubkey)
